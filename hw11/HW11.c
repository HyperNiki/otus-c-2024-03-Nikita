...
void print_crc32(const char* filename) {
    ...
    size_t read_by = CHUNK_SZ;

    uint8_t* ptr = (uint8_t*)mmap(NULL, f_st.st_size, PROT_READ, MAP_SHARED, fd, 0);
    if (ptr == MAP_FAILED) {
    ...
    while (bytes_readed < f_st.st_size) {
        ...
        crc = crc32(ptr + bytes_readed, read_by, crc);
        bytes_readed += read_by;
    }
    crc ^= ~0U;

    if (munmap(ptr, f_st.st_size) < 0) {
